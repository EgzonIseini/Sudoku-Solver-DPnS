...
def extract_sudoku_grid_from_image(image):
    sudoku_image = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
    ...
    if area < 10000:
        ...
        if idx % 9 == 0:
            ...
            for (cell_idx, cell) in enumerate(cells, 1):
                ...
                # Dividing width and height should give an approximately small aspect ratio if digit is a square.
                # If not ignore it.
                if (digit.shape[1] / digit.shape[0]) > 3:
                    continue

                if all(np.all(np.take(digit, index, axis=axis) == 0) for axis in range(digit.ndim) for index in (0, -1)):
...
for cell in sudoku_cells.values():
    if cell.is_empty is True:
        ...
        number_image = cv.putText(number_image, str(cell.cell_value), origin, cv.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0),
                                  2, cv.LINE_AA)

number_image = cv.resize(number_image, (original_sudoku_size[1], original_sudoku_size[0]))
image_points = np.array([(0, 0), (original_sudoku_size[1], 0), (original_sudoku_size[1], original_sudoku_size[0]),
