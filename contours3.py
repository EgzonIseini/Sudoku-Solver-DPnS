...
def extract_digit(img, rect, size):
    ...
    if w > 0 and h > 0 and (w * h) > 100 and len(digit) > 0:
        return scale_and_centre(digit, size, 8)
    else:
...
digits = extract_sudoku('sudoku1.jpg')
digits_to_recognize = []
for (i, digit) in enumerate(digits):
    if np.sum(digit) != 0:
        ...
        # if not cv2.imwrite(f'..\\images from sudoku\\Try 3\\cell{row}-{col}.jpg', digit):
        #     raise Exception("Could not write image")

input = np.array(digits_to_recognize)
th, input = cv2.threshold(input, 50, 255, cv2.THRESH_BINARY)
...
for row in range(nrows):
    for col in range(ncolumns):
        ...
        if i == len(digits_to_recognize):
            break
        th, im_th = cv2.threshold(digits_to_recognize[i], 50, 255, cv2.THRESH_BINARY)
